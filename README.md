...
- 响铃时弹出全屏提醒窗口
- **长按按键关闭本次闹钟**（带进度条显示）：长按的**按键可自定义**（默认空格，设置里点一下再按任意键即可改），**长按时长可调**（默认 5 秒，1–10 秒）

### ⚙️ 设置窗口
- 托盘图标右键 →「设置…」打开
- **通用**：开机自启、关闭窗口时的行为（询问 / 最小化到托盘 / 直接退出）、关闭响铃的长按按键与长按时长
- **铃声库**：收录数量、存储位置、快捷打开文件夹
- **关于**：版本、作者、GitHub 仓库、运行时版本

### 🎵 铃声管理（我的铃声）
- 从本地导入 mp3 / wav / ogg / m4a / flac 音频
- 自动复制到应用铃声目录，保留原名称（旧版本的 `alarm-` 前缀会在启动时自动清理）
- 铃声库支持：选择、试听（10 秒自动停止）、**重命名**、删除
- 音量滑块支持拖动和**滚轮调节**（步进 5）

### 🌐 内嵌音乐下载
- 铃声库内一键打开内嵌音乐网站窗口：<https://flac.music.hi.cn/>（地址可在 `src/main/main.ts` 的 `MUSIC_SITE` 常量里更换）
...
| 样式 | 原生 CSS（设计令牌驱动，颜色/圆角/阴影集中在 `:root`，主窗口与设置窗口共用） |
...
| 偏好设置（长按关闭按键 / 时长） | `%APPDATA%/tiny_alarm/preferences.json` | 同左 |
